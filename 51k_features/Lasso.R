...
#mrmodel <- biglm(selected.fmla, xydf.traincv)
#mrmodel <- biglm(terms(y ~ ., data=trainset), data=trainset)
Rprof(NULL) # ------------------------------------------------------------------ #

# Use corresponding object to make predictions and estimate the errors 
y.traincv.predicted <- predict(mrmodel, newdata = dfxy.traincv)
y.test.predicted    <- predict(mrmodel, newdata = dfxy.test)
